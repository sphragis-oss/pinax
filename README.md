...
- **Custom widgets via companion plugins**: a 20-line Obsidian plugin registers your widget with `window.pinax.registerWidget(...)`; copy-paste template in `examples/companion-widget-plugin/` and [AUTHORING.md](AUTHORING.md), TypeScript declarations in [`pinax-api.d.ts`](pinax-api.d.ts). Pinax itself never executes code from your vault.
